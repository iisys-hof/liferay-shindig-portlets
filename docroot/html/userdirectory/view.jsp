  ...
  */
  ...
  function <portlet:namespace/>callback(friends)
  {
    ...
    if(friends)
    {
      ...
      users.list.forEach(function(entry)
      {
        ...
        html += '<span class="label">' + <portlet:namespace/>NAME + '</span> '
          + '<b><a href="/web/guest/profile?userId='
          + entry.id + '" target="_blank">' + name + '</a></b><br/>';
        ...
        if(primary)
        {
          if(primary.title)
          {
            html += '<span class="label">' + <portlet:namespace/>POSITION + '</span> '
              + primary.title + '<br/>';
          }
      
          if(primary.department)
          {
            html += '<span class="label">' + <portlet:namespace/>DEPARTMENT + '</span> '
        ...
        if(entry.emails)
        {
          var length = entry.emails.length - 1;
          var index = 0;
    
          //html += '<img width="16" src="<%=request.getContextPath()%>/images/mail.png" '
          //  + 'alt="' + <portlet:namespace/>EMAILS + '" style="vertical-align: middle;" '
          //  + 'title="' + <portlet:namespace/>EMAILS + '" />&nbsp;';
          
          html += '<span class="label">' + <portlet:namespace/>EMAIL + '</span> ';
        ...
        if(entry.phoneNumbers)
        {
          ...
          //html += '<img width="16" src="<%=request.getContextPath()%>/images/telephone_call_contact.png" '
          //  + 'alt="' + <portlet:namespace/>PHONES + '" style="vertical-align: middle;" '
          //  + 'title="' + <portlet:namespace/>PHONES + '" />&nbsp;';
          
          html += '<span class="label">' + <portlet:namespace/>PHONE + '</span> ';
        ...
        if(entry.tags)
        {
          ...
          html += '<span class="label">Tags</span>: ';
        ...
        if(!<portlet:namespace/>fFriends[entry.id])
        {
          html += '<img width="32" src="<%=request.getContextPath()%>/images/user_male_add.png" '
            + 'alt="' + <portlet:namespace/>ADD_FRIEND + '" onclick="<portlet:namespace/>addFriend(\''
            + entry.id + '\')" title="' + <portlet:namespace/>ADD_FRIEND + '" '
        ...
        if(entry.id != <portlet:namespace/>USER_ID)
        {
          html += '<img width="32" src="<%=request.getContextPath()%>/images/send_email.png" '
            + 'alt="' + <portlet:namespace/>SEND_MESSAGE + '" title="' + <portlet:namespace/>SEND_MESSAGE + '" '
            + '/><br/>';
        }
      ...
      html += ' ' + (<portlet:namespace/>fFirst + 1) + ' ' + '<liferay-ui:message key="userdirectory-to" />'
        + ' ' + (<portlet:namespace/>fFirst + <portlet:namespace/>fMax)
        + ' ' + '<liferay-ui:message key="userdirectory-of" />' + ' ' + users.totalResults + ' ';
  ...
  function <portlet:namespace/>init()
  {
    ...
    else
    {
    	...
    	html += '<input type="text" style="display:table-cell;"';
        html += 'id="<portlet:namespace/>searchField" onkeypress="<portlet:namespace/>checkEnter(event)" placeholder="'
          + '<liferay-ui:message key="userdirectory-search-term" />' + '" value="' + searchValue + '"/>';
        
        html += '<div id="<portlet:namespace/>postSearchDiv"><div>'
          + '<select id="<portlet:namespace/>searchSel" style="width: 6em;">';
        ...
        html += ' ' + '<liferay-ui:message key="userdirectory-per-page" />'
          + ': <input type="text" style="width: 1.3em;"';
        html += 'id="<portlet:namespace/>maxField" value="' + <portlet:namespace/>fMax + '"/>';
        
        html += '<input type="button" value="'
          + '<liferay-ui:message key="userdirectory-go" />'
          + '" onclick="<portlet:namespace/>reset()"/></div></div>';
    }
